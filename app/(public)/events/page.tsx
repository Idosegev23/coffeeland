...
const features = [
  {
    ...
    title: 'ללא מאמץ',
    description: 'אנחנו מטפלים בהכל',
  },
  {
    lottie: '/lottie/birthday.json',
    title: 'חבילות גמישות',
    description: 'התאמה לכל תקציב',
  },
  {
    lottie: '/lottie/coffee.json',
    title: 'עוגה מעולה',
    description: 'מהטעימות שיש',
  },
  {
    lottie: '/lottie/gallery.json',
    title: 'רגעים מושלמים',
    description: 'זיכרונות לכל החיים',
  },
]
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl mx-auto text-center mb-12">
            ...
            <p className="text-lg text-text-light/80">
              אנחנו דואגים לכל הפרטים כדי שתוכלו פשוט ליהנות מהחגיגה
            </p>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {features.map((feature, index) => (
                <Card 
                  key={feature.title}
                  className={`${index === 2 ? 'col-span-2' : 'aspect-square'} rounded-tl-3xl rounded-tr-3xl rounded-bl-3xl rounded-br-none bg-[#4C2C21] border-[#4C2C21] flex ${index === 2 ? 'flex-row items-center gap-3 sm:gap-4' : 'flex-col justify-center'} p-3 sm:p-4`}
                >
                  {index === 2 ? (
                    <>
                      <div className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center flex-shrink-0">
                        <LottieIcon src={feature.lottie} />
                      </div>
                      <div className="flex-1">
                        <h3 className="text-sm sm:text-base font-semibold text-text-dark mb-0.5">{feature.title}</h3>
                        <p className="text-xs sm:text-sm text-text-dark/80 line-clamp-2">{feature.description}</p>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <h3 className="text-sm sm:text-base font-semibold text-text-dark text-center mb-1">{feature.title}</h3>
                      <p className="text-xs text-text-dark/80 text-center line-clamp-2">{feature.description}</p>
                    </>
                  )}
                </Card>
              ))}
